...
theta[l < 3] = theta[l < 3]*0.2 + 0.3; #dense community
theta[l >= 3] = theta[l >= 3]*0.02 + 0.03; #sparse communities
...
#    mu = 0.2;
  ...
  for(smalli in 1:K2){
    Qsmall[(smalli-1)*(smallp/K2)+(1:(smallp/K2)), smalli] = mu + 0.3; #remark. has a change here
  }


  signal_big = rbinom(bigp, 1, 0.05);#parameter changed
  ...
  # CA-clustering
  est_casc = CAclustering(A, Dbig, K1,  alphan = 10)
  ...
  est_net <- Net_based(Adj = A, K = K1, tau = log(n))
  ...
  # CA-clustering
  est_casc = CAclustering(A, Dsmall, K1,  alphan = 10)
  comp = table(est_casc, l);
  err_casc = cluster(comp)$error;
